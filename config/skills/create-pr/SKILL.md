...
添付するファイルがある場合（手順7で確認する）、本文の該当箇所（多くは「動作確認」）に `![代替テキスト](<絶対パス>)` で参照を書く。
`gh pr create --attach` が同じファイルへの参照をアップロード後の URL に書き換える。参照が無いファイルは本文末尾に追記されるため、置き場所を決めたいものは必ず参照を書く。
パスは絶対パスにする。gh は本文中のパスと `--attach` のパスをそれぞれカレントディレクトリ基準で絶対パス化して突き合わせるため、相対パスだと本文ファイルの置き場所とずれて書き換えられない。

- 対応形式: png / jpg / jpeg / gif / webp / svg（10MB まで）、mp4 / mov / webm（100MB まで）
- 動画は `![]()` で参照するとプレイヤーとして埋め込まれ、代替テキストは付けられない

### 7. タイトルの確認

差分から PR タイトル案を作り、ユーザーに提示してタイトルを確認する。ユーザーごと・リポジトリごとに好みが違うため、案をそのまま使わず必ず確認を取る。
本文もあわせて提示し、修正があれば反映する。
あわせて、添付する画像・動画（スクリーンショットや録画）があるか聞き、あればファイルパスをもらって手順6の要領で本文に反映する。UI の変更など見た目で確認できる差分では特に聞く。
...
添付ファイルがある場合は、ファイルごとに `--attach <絶対パス>` を足す（代替テキストは本文の参照に書いたものが使われる）。

```bash
gh pr create --draft --base <base> --title "<title>" --body-file <body-file> --assignee @me \
  --attach /abs/path/before.png --attach /abs/path/after.png
```

- `--attach` は gh v2.99.0 以降でのみ使える。`unknown flag: --attach` で失敗したら、gh の更新が必要だとユーザーに伝え、添付なしで作るか確認する
- 一部のファイルだけアップロードに失敗しても PR は作られ、コマンドは非ゼロで終わる。stdout に出た URL を拾い、失敗したファイルとあわせて報告する
